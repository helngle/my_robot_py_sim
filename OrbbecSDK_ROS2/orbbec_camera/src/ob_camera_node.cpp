...
void OBCameraNode::getParameters() {
  ...
  setAndGetNodeParameter<int>(publish_timestamp_offset_us_, "publish_timestamp_offset_us", 0);
  ...
  setAndGetNodeParameter<bool>(sync_color_depth_timestamp_, "sync_color_depth_timestamp", false);
...
void OBCameraNode::publishDepthPointCloud(const std::shared_ptr<ob::FrameSet> &frame_set) {
  ...
  auto frame_timestamp = getPublishTimestampUs(depth_frame);
...
uint64_t OBCameraNode::getPublishTimestampUs(const std::shared_ptr<ob::Frame> &frame) {
  auto raw_timestamp_us = getFrameTimestampUs(frame);
  auto adjusted_timestamp_us = static_cast<int64_t>(raw_timestamp_us) +
                               static_cast<int64_t>(publish_timestamp_offset_us_);
  return adjusted_timestamp_us > 0 ? static_cast<uint64_t>(adjusted_timestamp_us) : 0;
}

uint64_t OBCameraNode::getFrameSetSyncTimestampUs(const std::shared_ptr<ob::FrameSet> &frame_set) {
  if (!frame_set) {
    return 0;
  }
  auto depth_frame = frame_set->depthFrame();
  auto color_frame = frame_set->colorFrame();
  if (depth_frame && color_frame) {
    // Use depth timestamp as the common timestamp for synchronized depth+color pair.
    return getPublishTimestampUs(depth_frame);
  }
  if (depth_frame) {
    return getPublishTimestampUs(depth_frame);
  }
  if (color_frame) {
    return getPublishTimestampUs(color_frame);
  }
  return 0;
}
...
void OBCameraNode::onNewFrameSetCallback(std::shared_ptr<ob::FrameSet> frame_set) {
  ...
  try {
    ...
    const uint64_t synced_color_depth_timestamp_us =
      sync_color_depth_timestamp_ ? getFrameSetSyncTimestampUs(frame_set) : 0;
    ...
    for (const auto &stream_index : IMAGE_STREAMS) {
      if (enable_stream_[stream_index]) {
        ...
        const bool use_synced_ts =
            sync_color_depth_timestamp_ && frame_type == OB_FRAME_DEPTH &&
            synced_color_depth_timestamp_us > 0;
        onNewFrameCallback(frame, stream_index,
                           use_synced_ts ? synced_color_depth_timestamp_us : 0);
      }
    }
  } catch (const ob::Error &e) {
...
void OBCameraNode::onNewColorFrameCallback() {
  while (enable_stream_[COLOR] && rclcpp::ok() && is_running_.load()) {
    ...
    uint64_t forced_timestamp_us = 0;
    if (sync_color_depth_timestamp_) {
      forced_timestamp_us = getFrameSetSyncTimestampUs(frameSet);
    }
    onNewFrameCallback(frameSet->colorFrame(), COLOR, forced_timestamp_us);
...
void OBCameraNode::onNewFrameCallback(const std::shared_ptr<ob::Frame> &frame,
  ...
  auto frame_timestamp = forced_timestamp_us > 0 ? forced_timestamp_us : getPublishTimestampUs(frame);
  auto timestamp = fromUsToROSTime(frame_timestamp);
  if (!device_) {
