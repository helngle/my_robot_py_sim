...
class OBCameraNode {
 ...
 private:
  ...
  uint64_t getPublishTimestampUs(const std::shared_ptr<ob::Frame>& frame);

  uint64_t getFrameSetSyncTimestampUs(const std::shared_ptr<ob::FrameSet>& frame_set);
 ...
 private:
  ...
  int publish_timestamp_offset_us_ = 0;
  ...
  bool sync_color_depth_timestamp_ = false;
